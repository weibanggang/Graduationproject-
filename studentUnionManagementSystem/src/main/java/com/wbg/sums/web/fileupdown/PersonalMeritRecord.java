...
public class PersonalMeritRecord {
    @RequestMapping(value = "/personalMeritRecord", method = RequestMethod.POST, produces = "application/json;charset=utf-8")
    public Result upload(@RequestPart("file") MultipartFile multipartFile, HttpServletRequest request) {
        //取得相对路径
        String basePath = request.getServletContext().getRealPath(File.separator + "file" + File.separator + "personalMeritRecord" + File.separator);
        String rekativePath;
        try {
