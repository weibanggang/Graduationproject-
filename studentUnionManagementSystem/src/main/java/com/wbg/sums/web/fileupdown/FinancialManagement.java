...
public class FinancialManagement {
    @RequestMapping(value = "/financialManagement", method = RequestMethod.POST,produces = "application/json;charset=utf-8")
    public Result upload(@RequestPart("file") MultipartFile multipartFile, HttpServletRequest request){
        //取得相对路径
        String basePath = request.getServletContext().getRealPath(File.separator+ "file" + File.separator + "financialManagement"+File.separator);
        String rekativePath;
        try {
