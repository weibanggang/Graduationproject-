...
public class MinutesOfTheMeeting {
    @RequestMapping(value = "/minutesOfTheMeeting", method = RequestMethod.POST, produces = "application/json;charset=utf-8")
    public Result upload(@RequestPart("file") MultipartFile multipartFile, HttpServletRequest request) {
        //取得相对路径
        String basePath = request.getServletContext().getRealPath(File.separator + "file" + File.separator + "minutesOfTheMeeting" + File.separator);
        String rekativePath;
        try {
